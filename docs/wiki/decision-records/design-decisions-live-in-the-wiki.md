...
## Context
For its first weeks this system recorded its own architecture decisions the conventional way: an ADR appendix pinned to the bottom of each package's design document. That form has no status or lifecycle, no link from a decision to the one that revises it, and identity that is only per-file — wiki-mcp and wiki-models each shipped a decision with the same number, a collision a single namespace makes impossible. There was no way to ask which decisions touch the read model, or to follow a decision's consequences.
...
This record is itself the proof: it was authored in the wiki, not migrated from a design-document appendix — which is why, alone among the records here, it carries no legacy id.

## Consequences
Design decisions are now searchable, cross-linked, and governed objects inside the very system they govern. Deterministic render keeps a future docs snapshot churn-free (a separate Markdown-projection feature). The design-document appendices are retired only once that snapshot lands, so there is never a window with two sources of truth — nor one with none.

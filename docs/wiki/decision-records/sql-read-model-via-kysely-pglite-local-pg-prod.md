...
section model the core tables are the section
tree (sections(pageid, parentid, key, ord, fields JSONB, meta JSONB)), with list elements and
blocks/inline trees living inside field JSONB — not a separate flat fields/items pair.
...
queryable; typed per-type tables remain an opt-in optimization.

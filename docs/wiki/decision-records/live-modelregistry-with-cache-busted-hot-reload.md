...
## Context
Page-type schema must be swappable at runtime so a model can be edited, rebuilt, and reloaded into a running server (the local edit then build then reload loop, driven from a build pipeline — never by an agent). Today createWikiMcp takes a fixed pageTypes set and builds an immutable Registry once; the projection captures registry and fingerprint at construction and EmbeddedEngine binds the set per hot handle. Page types are authored in wiki-models and loaded by reference, and the engine is already version-aware (upcast-to-latest).
...
- the engine Registry is rebuilt from the new def set, EmbeddedEngine.rebind rebuilds the engine from
  ...
  all from the stream with the new registry.
...
pipeline-driven, not an MCP tool. wiki-server
...
correctness rides the projection fold (upcasting, unknown-type halt) for free. Cache-busting is the minimal
...
version halts affected workspaces loudly. The bundle must
...
trusted). Per-namespace model selection + persistence stay reserved.
...
actually changed; that column is still stamped on every apply.

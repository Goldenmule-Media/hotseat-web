...
Render was a per-type render(page, ctx) => markdown function — the
...
## Consequences
The page-type definition loses apply/render and gains sections/elements plus a
render config, render becomes a read model, and a render read-model module exposed as a wiki/render
subpath is added. The render-config vocabulary must cover today's bespoke layouts (e.g. the
open/resolved question split) without becoming "config that is secretly code" — an open item
still to specify. Golden render tests are rewritten against the config.

#!/usr/bin/env node
/**
 * Standalone `bin` entry for `wiki-mcp`. Runs the library's {@link main} over stdio
 * with NO page types registered (a host injects real ones).
 *
 * This is kept SEPARATE from `./main` (the library) on purpose: `wiki-server` embeds
 * `wiki-mcp` by bundling it FROM SOURCE. When a bundler inlines many modules into
 * one file they all share that file's `import.meta.url`, so an
